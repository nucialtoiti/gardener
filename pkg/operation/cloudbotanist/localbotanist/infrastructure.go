...
func (b *LocalBotanist) DeployInfrastructure() error {
	...
	chart, err := b.ChartApplierSeed.Render(filepath.Join(common.ChartPath, "shoot-cloud-config", "charts", "downloader"), "shoot-cloud-config-downloader", metav1.NamespaceSystem, config)
	if err != nil {
		return err
	}
